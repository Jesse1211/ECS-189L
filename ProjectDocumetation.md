...
- Enhanced the `BattleScene` by expanding the range of the grid using `TileMap`: I increased the grid size of the `BattleScene` to accommodate more complex and larger-scale battles. This was achieved by utilizing the TileMap, allowing for a flexible grid system that can adjust to various battle scenarios and gameplay requirements, including modifying various types of special interactive ground, like elevators, walls, and fly dragon. Besides, expanding the battle scene increases playability in content and riches players' experience. Players can go through the scene with different experiences because of different designs.

- Added multiple responsive prefabs with scripts for `Boat`, `Elevator`, `Death Zone`, `Dragonfly`, `Climable Wall`, `BloodTrees`, `Orb Plant`. I incorporated a multitude of responsive prefabs including vehicles like `Boat` and `Elevator`, which are developed in the MovingPlatform that basically object that can patrol between points, environmental hazards such as `Death Zone`, originally created in the DeathZone scripts, creatures like `Dragonfly`, physical obstacles like `Climable Wall` (This part is in the player controller), and unique game elements such as `BloodTrees` and `Orb Plant`, created in the OrbController that object can emit harmful red and healing white orb. Each of these prefabs was endowed with unique behavior and interaction scripts, thereby significantly enriching the game world's interaction dynamics. For example, the `Death Zone` will be harmful to players to interact with. On the other hand, `Elevator` will be a good tool to explore the world, but `Orb Plant` would probably harm the player or help the player. [issue] (https://github.com/Jesse1211/ECS-189L/issues/23).

- Added Dialog feature by using `Canvas`, `Image`, `Button`, `Text`, and `Scripts` which helps to achieve typing effect. I created a new Dialog feature to promote interactive storytelling in the game. This feature was implemented using a blend of Unity UI elements such as `Canvas`, `Image`, `Button`, and `Text`, along with custom scripts for dialogue control (UI picture was made by others). By using the images generated by group mates, I made a good-looking dialog interface, with black-green color and a nice margin, which fit the ambiance of the game and enhance the experience of playing the game. The typing effect added to the dialogues gave them a more engaging and realistic feel, thus enhancing the player's immersion in the game narrative. I also customize the dialog speed by controlling the speed of the appearance of each letter in a sentence. [issue](https://github.com/Jesse1211/ECS-189L/issues/36).

- Implemented animation states for most models including `elk`, `fox`, `crow`, `rats`, `Fanboy`, and `Gunslinger`. I introduced distinctive animation states to a majority of the game models. These included a variety of animal characters like `elk`, `fox`, `crow`, and `rats`, as well as human characters such as `Fanboy` and `Gunslinger`. The animation states provide each model with unique movements and behaviors, adding depth to their characteristics and making the game world more believable. Also, To make them look smooth, I need to arrange the correct intervals between pictures. I repeatedly try several times and finally make a good one. To make sure they appropriately show the same size in the game world, I adjusted `pixel per unit` every time when I make them. So basically now those characters will not be shrinked or expanded. [issue](https://github.com/Jesse1211/ECS-189L/issues/26).

- Scene changing for some scenes. Considering the content of the Floatingisland, it is better to place the scene-changing spot further at the right of the map because I think there should be more room for the player to fight with the enemy. If not, it would be too restricted to fight, which influences the experience of game playing. Also, the player should be spawned at the side instead of the middle. This will make the game sequential.

- Bag item functioning. The bag should have a function that can pick up the weapon and take it as a weapon to shoot. So, I made weapons that can be picked up and equipt on weapon slots. When pressing 1, 2, 3, 4, it will accordingly emit the weapons in each slot. Of course, it would cause some damage to the enemy. Especially for those master weapons, it will be the key to defeat the boss.

- Secondary drafted animator controller for the main character: I took the lead in creating a second draft for the animator controller of our main character. This was a thorough revision that incorporated feedback from the first draft and improved character movements, transitions, and controls to achieve a more natural and responsive animation flow. In detail, the secondary we tried to use a trigger to transit the state between animations. We did it; however, it is not efficient to use, and tried to modify it later.

- Enemy AI:
  - Designed enemy AI by using FSM which sets `Idle`, `Patrol`, `Chase`, `React`, `Attack`, `Death`, `IsHit`, and `teleport` states that make the enemy more responsive to the player: Each state defined unique behaviors, making the enemy AI responsive and adaptive to player actions and game events. In detail, `Idle` serves for the standing without any actions, but it will transit to `Patrol` state. This is an enemy need to patrol over time. If he found the player, he will enter in `React` state. This is a state for transiting the state. The states include `Attack`, `Patrol`. The rest will be easy to understand, `Death` and `IsHit`. The interesting part is `teleport` because it is a mechanism for strengthening enemies, increasing the difficulty of playing this game. So, this is for the boss mechanism, the player would find that it is more difficult to against other enemies.

  - To each state, I attached key methods such as `OnEnter()`, `OnUpdate()`, `OnExit()`, `GetHp()`, and `SetUp()` using an interface, which efficiently passes the information between each state. This approach streamlines transitions between states and provides real-time updates to the enemy's behaviors, significantly boosting the AI's functionality and responsiveness. 
...
### Zuge Li
- In game testing and the overall game feel, I engaged in a comprehensive play-through of our game upon its completion. During this exploration, I noticed several aspects that can be adjusted. Our main character, a young girl known as KK, encountered a boss in a rather limited space. This restrictive area negatively impacted the gameplay feeling, that players will feel this is unreasonable and hard to play with, prompting me to redesign the environment to afford more tactical depth. I expanded the area and incorporated additional terrain features such as islands, thus enriching the battleground and facilitating improved combat sequences.
- When playing, one thing that violates the play feel is that the player was spawned at the center of the map. This does not make sense at all. It is easy to make players confused because players do not know where to go. To fix this, it is better to drop the player at the left of the map and play along with the map. This is sequential and players will have strong targets on this map.
- While enhancing the battle scenes, I also recognized the need for alternate routes toward the objectives. The introduction of these paths would not only offer players more strategic options but also contribute to the diversity of gameplay, limiting direct confrontations with enemies.
- The boss battles previously lacked the necessary hit animations, leaving the player without essential feedback on successful strikes. To rectify this, I incorporated a relevant animation feature, providing the player with the needed visual cues.
- However, other areas also require careful attention, particularly the box colliders that result in an odd gaming experience. As it stands, these colliders appear when players come into contact with non-essential characters, making it a strange encounter. I have revised these elements, allowing players to pass freely through these characters.
- In the context of an RPG, dialogue between characters is crucial to maintaining player immersion. As such, I've fine-tuned the text speed to simulate a natural conversation pace, lending authenticity to the character interactions. Also for the sake of simplicity, I think that it is better to use fixed objects that the little girl can talk with, e.g. Deer.
- My team member, Jesse, and I also addressed issues with the tilemap. We observed that players could traverse these tiles, which was not the intended function. After numerous trials, we opted for a full-block solution to secure the map's borders, thus reinforcing the game's stability.
- Finally, I noted that the teleportation points were ineffective and diminished the overall gameplay experience. It's essential for these elements to be distinctive and visually appealing. Initially, a windmill represented these points, but I've transformed it into a more striking flydragon figure. This symbolizes the dragon's ability to whisk players to new scenes, thus providing a more intuitive and immersive teleportation feature.
